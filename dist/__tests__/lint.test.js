...
// ── Утилита ────────────────────────────────────────────────────────────────────
function codes(children) {
    return (0, lint_1.lintChildren)(children).map(i => i.code);
}
// ── Пустые страницы ────────────────────────────────────────────────────────────
...
describe("too-many-blanks", () => {
    ...
    test("4 blank подряд → одно предупреждение (не 4)", () => {
        const issues = (0, lint_1.lintChildren)([blank(), blank(), blank(), blank()]);
...
describe("чистый документ", () => {
    test("нормальная структура не даёт предупреждений", () => {
        ...
        expect((0, lint_1.lintChildren)(children)).toHaveLength(0);
    });
});
